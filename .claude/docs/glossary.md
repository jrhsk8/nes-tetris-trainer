...
The movable translucent preview of a piece that the player positions to a final resting placement, then confirms. **Left/right is free column selection**, never blocked by the stack. Since grill-with-docs #7 (2026-06-23) a lateral press moves the piece to the **reachable position in the target column nearest the current row, preferring at-or-below** — so it **tucks into a pocket** when you press toward one from nearby and **rides up** over a wall only when nothing at-or-below is reachable (a press fails only when the column is full to the very top, the "off-screen" line). This is **tuck-seeking** lateral; it replaced the grill-with-docs #6 *ride-up-to-the-top* rule, which ejected the piece out of any pocket you pressed toward unless you had soft-dropped to the exact row — the "can't tuck the J into the col-4/col-8 holes" report. The model and input are **navigation-complete** (every generator-enumerated [resting placement](#tuck--spin) is reachable by some input sequence), so tucks/spins remain first-class. Inputs: left, right, rotate, soft-drop, and the soft-drop inverse "move up". On touch devices the same positioning is a **drag anywhere on the board** (finger column → piece column, with the ▲/▼ soft-drop buttons for tuck depth) committed by an explicit **Confirm** button. Every reachable position is in the generator's BFS reachable set, so the player can never confirm a placement the generator did not enumerate (the superset binding invariant, [tuck / spin](#tuck--spin)). There is no timer, so input is still "pick a resting placement," not real-time play. (Not the standard hard-drop drop-shadow that "ghost" usually means in Tetris. 2026-06-23: free/ride-up narrowed to tuck-seeking (nearest reachable row, prefer at-or-below) after "can't tuck the J" reports. 2026-06-22: lateral changed from collision-gated to free/ride-up after "L/R won't move" reports. 2026-06-21: was hard-drop / column-only, which couldn't express tucks.)
