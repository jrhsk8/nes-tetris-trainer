...
The movable translucent preview of a piece that the player positions to a final resting placement, then confirms. **Left/right is free column selection** (since grill-with-docs #6, 2026-06-22): never blocked by the stack — the piece **rides up** over any wall to the highest row that fits in the target column, bounded only by the playfield walls, so a lateral press fails only when the whole column is full to the very top (the "off-screen" line). Sliding *into* an open pocket is already reachable at the current row, so **tucks/spins** still work — they are now reached by **soft-dropping into the gap first**, where lateral movement stays collision-gated at depth. Inputs: left, right, rotate, soft-drop, and the soft-drop inverse "move up". On touch devices the same positioning is a **drag anywhere on the board** (finger column → piece column, with the ▲/▼ soft-drop buttons for tuck depth) committed by an explicit **Confirm** button. Every reachable position is in the generator's BFS reachable set, so the player can never confirm a placement the generator did not enumerate (the superset binding invariant, [tuck / spin](#tuck--spin)). There is no timer, so input is still "pick a resting placement," not real-time play. (Not the standard hard-drop drop-shadow that "ghost" usually means in Tetris. 2026-06-22: lateral changed from collision-gated to free/ride-up after "L/R won't move" reports. 2026-06-21: was hard-drop / column-only, which couldn't express tucks.)
